...
internal class SystemMethods
{
    ...
    public SystemMethods(LuaState engine, IClientService clientService, IDataService dataService, IVariableService variableService, IAutomationHandler automationHandler, int? topAutomationId)
    {
        ...
        engine.Environment["system_log"] = new LuaFunction("system_log", async (context, ct) =>
        {
            var instanceId = context.GetArgument<string>(0);
            var message = context.GetArgument<string>(1);
            await _automationHandler.AddLogAsync(instanceId, message);
            return 1;
        });
    }
